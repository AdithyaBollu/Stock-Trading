...
- tail of memory/TRADE-LOG.md (find most recent EOD snapshot → yesterday's equity for Day P&L)
...
- Cash % (target: ≤5% — flag if exceeded)
...
**Notes:** one-paragraph plain-english summary. Flag if cash > 5% and why it wasn't deployed.

STEP 5 — Send ONE Discord message (always, even on no-trade days). <= 15 lines:
bash scripts/discord.sh "EOD MMM DD
Portfolio: \$X (±X% day, ±X% phase)
Cash: \$X (X%)$([ X > 5 ] && echo ' ⚠️ OVER 5%')
